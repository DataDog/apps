...
_users.post = (data, callback) => {
    ...
    if (!name) {
        callback(400, {
            error: 'Request should include name in its request body'
        });
        return;
    }
    ...
    if (!helpers.validatePassword(password)) {
        callback(400, {
            error:
                'Request should include a valid password (min. 5 characters long)'
        });
        return;
    }
            ...
            if (!err) {
                _data.create('users', `${email}-cart`, [], err => {
                    if (!err) {
                        ...
                        console.log(err);
                        callback(500, {
                            error: "Could not create the new user's cart"
                        });
                    }
                });
            } else {
            ...
            _data.read('users', email, function (err, userData) {
                if (!err && userData) {
                    _data.delete('users', email, function (err) {
                        if (!err) {
                            _data.delete('users', `${email}-cart`, function (
                                err
                            ) {
                                if (!err) {
                                    callback(200);
                                    ...
                                    callback(500, {
                                        error:
                                            'Could not delete cart of specified user'
                                    });
                                }
                            });
                        } else {
...
_tokens.post = function (data, callback) {
    ...
    if (!email) {
        callback(400, {
            error: 'Request should include a valid email address'
        });
        return;
    }
        ...
        if (!err && tokenData) {
            ...
            if (expires > Date.now()) {
                ...
                _data.update('tokens', id, tokenData, function (err) {
                    if (!err) {
                        callback(200, {
                            data: { expirationDate: tokenData.expires }
                        });
                    } else {
                        console.log(err);
                        callback(500, {
                            error: "Unable to update token's expiration"
                        });
                    }
                });
            } else {
...
_cart.post = (data, callback) => {
    ...
    if (!size) {
        callback(400, {
            error:
                'Size missing or invalid (available options: 10, 12, 14, 16, 18)'
        });
        return;
    }
                ...
                if (!err && menuData) {
                    ...
                    _data.read('users', `${email}-cart`, (err, cartData) => {
                        if (!err && cartData) {
                            ...
                            callback(400, {
                                error: 'Could not load cart content'
                            });
                        }
                    });
                } else {
                    callback(400, {
                        error:
                            'Generating a cart overview failed, could not load menu'
                    });
                }
            });
        } else {
...
_cart.delete = (data, callback) => {
    ...
    const index =
        !isNaN(payload.index) && payload.index % 1 === 0
            ? payload.index
            : false;
    const token = typeof headers.token === 'string' ? headers.token : false;
                                    ...
                                    helpers.createStripePayment(
                                        ...
                                        err => {
                                            if (!err) {
                                                ...
                                                const emailBody = helpers.createEmailBody(
                                                    order
                                                );
