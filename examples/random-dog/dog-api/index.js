...
app.get('/image', async (req, res) => {
    ...
    const url =
        breedId === 0 ? urlBase : urlBase.concat(`?breed_id=${breedId}`);
    const imageUrl = await axios
        ...
        .then(response => {
            if (response.data.length === 0) {
                return 'https://upload.wikimedia.org/wikipedia/en/thumb/7/7e/Datadog_logo.svg/1200px-Datadog_logo.svg.png';
            }
